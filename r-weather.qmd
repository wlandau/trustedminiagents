...
author: "Will Landau"
description: >
  A worked example that adds tools to the R Shiny template, building a
  complete trusted mini-agent for checking city weather.
---

# Example: weather agent {#sec-weather}

This chapter builds on the template in [the previous chapter](r-template.qmd) and shows how to add tools to a trusted mini-agent in a Shiny app.

We use the weather agent example from [Chapter 2](errors.qmd) to make the discussion concrete.
...
![Screenshot of the completed weather app showing a shinychat interface on the left with a user query about weather, a leaflet map in the center displaying the queried location, and a weather data table on the right with temperature and conditions.](./images/r-weather/app.png)

In the `shinychat` interface on the left, the user asks for the weather in a US city.
The leaflet map in the middle lets the user easily check if the AI-generated coordinates for the city are correct.
...
2. The leaflet map in the middle is super easy to check, so we can be confident the app is checking the weather in the right place.
...
flowchart LR
  ...
  subgraph Human oversight
    RV --> M["leaflet map of<br>model-generated lat and long"]
  end
```
...
At its discretion, the AI model sends [`ellmer`](https://ellmer.tidyverse.org/) the text for a tool call (e.g. `get_weather(41.8, -87.6)`), at which point [`ellmer`](https://ellmer.tidyverse.org/) runs the tool call instead of simply relaying the text back to [`shinychat`](https://posit-dev.github.io/shinychat/r/).
...
Neither the user nor the AI model can update those reactive values any other way.

The leaflet map renders the coordinates the AI model picked, so the user can check whether its resolution of the query looks correct.
...
The leaflet map helps the user check the correctness of AI-generated location data.
...
As before, we write a constructor for the chat object.
...
![Screenshot of the shinytest2 recording interface in a browser window at the start of recording, showing the empty app and test recording controls.](./images/r-weather/test1.png)
...
* After the leaflet map and weather data table populate, click "Expect Shiny values" again.
Your window should now look like this:

![Screenshot of the shinytest2 recording interface after test interactions, showing the populated leaflet map and weather data table along with recorded Shiny values.](./images/r-weather/test2.png)
...
2. To avoid race conditions with AI model streaming, add `app$wait_for_idle()` before each call to `app$expect_values()`.
