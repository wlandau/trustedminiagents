...
This chapter builds on the template in [the previous chapter](template-py.qmd) and shows how to add tools to a trusted mini-agent in a Shiny for Python app.
We use the same weather agent idea as [the R version of this chapter](weather.qmd) to make the discussion concrete.
...
It is the same trust structure as [the R weather agent](weather.qmd#app-design), just with Python components.
